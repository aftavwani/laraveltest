...
<div class="row">
    ...
    <div class="col-lg-2">
        ...
        @endif
        <hr/>
        <img src="{{asset(Config::get('gallery.upload_folder').$photo->url)}}"
             class="img-rounded img-responsive img-uploaded" alt=""/>
        @if(in_array($photo->id, $album->photos->lists('id')))
        {{ Form::open(array('method' => 'POST', 'action' => array('App\Gallery\Controllers\AlbumsController@removePhoto') )) }}
        {{ Form::submit('Remove from album', array('class' => 'btn btn-danger btn-xs')) }}
        @else
        {{ Form::open(array('method' => 'POST', 'action' => array('App\Gallery\Controllers\AlbumsController@addPhoto') )) }}
